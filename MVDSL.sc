...
object DSLDef {
  ...
  def eval(env : Environment, expr : Expr) : Value = expr match {
    ...
    case Sequence(x) => x match {
      ...
      case Cons(x, next)  => x match {
        ...
        case LetRec(f, param, body, prgm) => eval(ExtendRec(f, param, body, env), Sequence(Cons(prgm, next)))
        case _                            => eval(env, Sequence(next))
      }
    }
  }
  ...
  def main(args: Array[String]) {
    ...
    val rfsq:Expr = Sequence(Cons(LetRec("f","x",
                                         IfThenElse(
                                           Eq("x",one),
                                           one,
                                         ...
                                         Zero
                                  ), Cons(FunCall("f", three), Empty)))
    println(eval(EmptyEnv, rfsq))
  }
}
